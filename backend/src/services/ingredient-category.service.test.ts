...
import { IngredientCategoryService } from "./ingredient-category.service.js";
...
describe("IngredientCategoryService", () => {
  it("list() scopes to the given restaurantId only", async () => {
    ...
    const service = new IngredientCategoryService(repository as never);
  ...
  it("create() creates the category under the given restaurantId, never a client-supplied one", async () => {
    ...
    await service.create("rest-1", { name: "Cheese" });

    expect(repository.create).toHaveBeenCalledWith({ restaurantId: "rest-1", name: "Cheese" });
  });
