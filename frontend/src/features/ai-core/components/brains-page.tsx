...
export function BrainsPage() {
  ...
  return (
    <Main className='space-y-6'>
      <PageHeader
        title='Brains'
        description='Reusable AI configurations — provider, model, prompt, and routing policy. One or more Capabilities may share a Brain.'
        breadcrumbs={[
            ...
            <TableBody>
              {brains.length === 0 ? (
                <TableRow>
                  ...
                  >
                    No Brains yet — create one, then open it to add a Routing
                    Policy and a Prompt version.
                  </TableCell>
                </TableRow>
              ) : (
                brains.map((brain) => (
                  <TableRow key={brain.id}>
                    ...
                    <TableCell>
                      <Badge
                        variant={brain.isEnabled ? 'default' : 'secondary'}
                      >
      ...
      <BrainDialog
        open={editingBrain !== null}
        onOpenChange={(open) => !open && setEditingBrain(null)}
        brain={editingBrain}
      />
    </Main>
  )
