...
export function PermissionMatrix({
  ...
  const toggle = (permissions: string[], checked: boolean) =>
    onChange(
      checked
        ? [...new Set([...value, ...permissions])]
        : value.filter((permission) => !permissions.includes(permission))
    )
  return (
        ...
        <section key={category.label} className='space-y-3'>
          ...
          <div className='grid gap-3 rounded-md border p-4 sm:grid-cols-2'>
            {category.features.map((feature) => {
              const checked = feature.permissions.every((permission) =>
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(value) =>
                      toggle(feature.permissions, value === true)
                    }
                  />
